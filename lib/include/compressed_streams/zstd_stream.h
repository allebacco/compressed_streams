...
protected:
    ...
    std::streamsize xsputn(const char_type* data, std::streamsize data_size) override
    {
        ...
        {
            ZSTD_outBuffer output = { m_tmp_buffer.data(), m_tmp_buffer.size(), 0 };
            size_t toRead = ZSTD_compressStream(m_zstd_cstream, &output , &input);   /* toRead is guaranteed to be <= ZSTD_CStreamInSize() */
        ...
        return bytes_written; // returns the number of characters successfully written.
    }

    int_type overflow(int_type ch) override
    {
        const char data = traits_type::to_char_type(ch);
        return static_cast<int_type>(xsputn(&data, 1)); // returns the number of characters successfully written.
    }
...
{
public:
    ZstdOStream(std::ostream &stream, int compression_level=ZSTD_CLEVEL_DEFAULT) :
